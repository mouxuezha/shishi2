...
class call_components:
    ...
    def call_matlab(self):
        ...
        try:
            ...
            strbuffer = 'MXairfoil : matlab time out. last step are used for continue. in  eventlet sec_timeout = '+str(sec_timeout)
            self.jilu(strbuffer)
            self.done=1
        finally:
            t.cancel()
        ...
        if((time_end-time_start)>sec_timeout):
            ...
            self.done = 1
            strbuffer = 'MXairfoil : matlab time out. last step are used for continue. in last sec_timeout = '+str(sec_timeout)
            self.jilu(strbuffer)
    ...
    def call_Turbo(self,**kargs):
        ...
        if((time_end-time_start)>sec_timeout):
            #even if I can't do exit, at least I can remember
            self.done = 1
            strbuffer = 'MXairfoil : calculating time out. last step are used for continue. sec_timeout = '+str(sec_timeout)
            self.jilu(strbuffer)
    ...
    def call_CFView(self):
        ...
        if jieguo!= 0:
                # which means matlab exits with exception
                self.done = 1
                strbuffer = 'MXairfoil : call_CFView running a loneliness'+ self.IGG_location
                self.jilu(strbuffer)
                return
        time_end = time.time()
        ...
        if((time_end-time_start)>sec_timeout):
            #even if I can't do exit, at least I can remember
            self.done = 1
            strbuffer = 'MXairfoil : CFView time out. last step are used for continue. sec_timeout = '+str(sec_timeout) + 'location is:' + self.script_folder
            self.jilu(strbuffer)
...
if __name__=="__main__":
    ...
    if flag == 0 :
        ...
        # x_rand = np.array([ 0.3564,-0.350176,0.46152,5.828])
        # x_rand = np.array([ 0.37112,-0.236256,0.43464, 7.0472  ])
        # x_rand = np.array([ 0.39608,-0.250848,0.38152,6.188])#
        x_rand = np.array([ 0.36584,-0.23088 ,0.46568  ,5.972  ]) # 355
        ...
        shishi.test_existing_case(13)
        shishi.call_matlab()
        shishi.call_IGG()
        shishi.call_Turbo()
        shishi.call_CFView()
        omega,rise = shishi.get_value()
        omega,rise,rise = shishi.get_value_new()
