...
class call_components:
    ...
    def __del__(self):
        ...
        try:
            ...
            shutil.rmtree(self.matlab_location)
            shutil.rmtree(self.script_folder)
            print('MXairfoil: successfully remove the temp file using __del__')
        except:
            print('MXairfoil: fail to remove the temp file in __del__')
    ...
    @time_out.time_out(60, time_out.callback_func) # this is only for os.system.
    ...
    def call_matlab(self):
        ...
        # dir_now = os.getcwd()
        # print(dir_now)
        # os.chdir('C:\Users\y\Desktop\MXairfoilCDA\code')
        os.chdir(matlab_location+'/code')
        dir_now = os.getcwd()
        print('MXairfoil: direction now is: ')
        print(dir_now)
    ...
    def call_Turbo(self):
        ...
        # rizhi = 'MXairfoil:debugging, \nreal location before euranus is \n '+ real_location + '\n excepted location is :'+self.script_folder
        # self.jilu(rizhi)

        os.chdir(self.script_folder) #It seems there are still little probability to wrong. even if chdir here .
    ...
    def call_CFView(self):
        ...
        if((time_end-time_start)>sec_timeout):
            #even if I can't do exit, at least I can remember
            self.done = 1
            strbuffer = 'MXairfoil : cfviewx time out. last step are used for continue. sec_timeout = '+str(sec_timeout)
            self.jilu(strbuffer)
    ...
    def get_value(self):
        ...
        try:
            ...
            print('MXairfoil: fail to get omega and rise')
            omega = 0
            rise = 0
        return omega,rise
